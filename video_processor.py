...
--- ORIENTATION ---
Choose ONE: straight | gay | shemale
- straight = male+female performers, OR female-only (lesbian goes in categories)
- gay = only male performers
- shemale = trans performer who has both penis and female features (pre-op)

--- DESCRIPTION ---
Write 3–5 sentences using raw explicit slang. Start directly with the action.
NEVER start with "This video", "In this scene", "The video", etc.

--- CATEGORIES ---
Select ONLY tags from this list (exact spelling required): {categories_list}
...
FORBIDDEN COMBINATIONS (auto-removed in post-processing anyway, but still):
  ...
  NO "Double Penetration" without two simultaneous penetrations visible
...
{{"categories":[...only NEW categories not already listed...]}}"""
...
SCORING CRITERIA — assign a score 1–10 to each frame you consider:
  ...
  -1 if image is blurry, dark, or overexposed
...
def extract_json_from_response(text: str) -> Optional[Dict]:
    ...
    # Убираем trailing-запятые перед ] или } (невалидный JSON)
...
def process_video(video_path: str, output_dir: str, base_name: str) -> Dict:
    ...
    try:
        ...
        raw1a = call_vision_model(
            ...
            frames_1a,
            {"temperature": 0.45, "top_p": 0.85, "max_tokens": 900}
        )
        ...
        cats_a       = _normalize_cats(p1a.get("categories", []))
