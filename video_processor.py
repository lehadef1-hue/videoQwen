...
#     Pass 1a — первые 25 кадров (4%–96%)  → orientation + description + categories_A
...
# ─── Промпт 3: финальный выбор из топ-20 кандидатов ──────────────────────────
...
def extract_json_from_response(text: str) -> Optional[Dict]:
    ...
    # 2.5 Truncated JSON: model cut off before closing braces
    if depth > 0 and start is not None:
...
def process_video(video_path: str, output_dir: str, base_name: str) -> Dict:
    ...
    try:
        ...
        # ── Мерж кандидатов: берём top-20 для финального прохода ────────────────
        all_candidates = sorted(
            candidates_a + candidates_b,
            key=lambda x: x["score"],
            reverse=True
        ...
        # ── Pass 3: финальный выбор из top-10 ────────────────────────────────────
        top5 = top10[:5]  # fallback
...
async def upload_videos(files: List[UploadFile] = File(...)):
    """Upload video files to the input directory."""
