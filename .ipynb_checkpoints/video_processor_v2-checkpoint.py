...
def _run_task(task_id: str, fn, *args, webhook_url: str = "", **kwargs):
    ...
    if webhook_url:
        try:
            requests.post(webhook_url, json={"task_id": task_id, **result}, timeout=15)
            logger.info(f"Webhook fired → {webhook_url}")
        except Exception as e:
...
def _api_task(task_id: str, req: AnalyzeRequest):
    ...
    # Process (first language only for now; multi-lang = multiple SEO passes)
    lang_code = (req.languages or ["en"])[0]
    ...
    result = process_video_v2(video_path, str(video_out), base_name, language, req.style)
...
async def api_analyze(
    ...
    t = threading.Thread(
        target=_run_task,
        args=(task_id, _api_task, task_id, body),
